#include <stdio.h>
#include <stdlib.h>
#include <string.h>

int main()
{
	int bLoop = 1;
	char *strLine[128] = {0}; // reset all value - Null
	int nTailIndex = 0;

	while (bLoop) {
		char *pTemp;
		char strCmd[128];
		gets(strCmd);

		pTemp = strtok(strCmd," ");

		if (!strcmp(pTemp,"exit")) { // == 0
			bLoop = 0;

		} else if (!strcmp(pTemp,"push")) { //add
			pTemp = strtok(NULL," ");
			
			char *pstr = (char *)malloc(strlen(pTemp)+1);
			strcpy(pstr,pTemp);
			strLine[nTailIndex++] = pstr;

		} else if (!strcmp(pTemp,"pop")) { // back delete
			nTailIndex--;
			free(strLine[nTailIndex]); // return assignment
			strLine[nTailIndex] = 0x00;
			
		} else if (!strcmp(pTemp,"del")) { // front delete
			free(strLine[0]);
			nTailIndex--;
			for (int i=0; i<nTailIndex+1; i++) {
				strLine[i] = strLine[i+1];
			}

		} else if (!strcmp(pTemp,"ins")) { // insert data
			//ins 2 hello
			//list2 - hello
			int nIndex = atoi(strtok(NULL," "));
			pTemp = strtok(NULL,"");
			printf("%d, %s add \r\n",nIndex,pTemp);
			
			int i;
			for (i=nTailIndex; i>nIndex; i--) {
				strLine[i] = strLine[i-1];
			}
			char *pstr = (char *)malloc(strlen(pTemp)+1);
			strcpy(pstr,pTemp);
			strLine[i] = pstr;
			nTailIndex++;

		} else if (!strcmp(pTemp,"rm")) { // delete wanted data
			//rm 2
			//line2 delete
			int nIndex = atoi(strtok(NULL," "));
			free(strLine[nIndex]); // return assignment
			for (int i=nIndex; i<nTailIndex; i++) {
				strLine[i] = strLine[i+1];

			}
			nTailIndex--;
			strLine[nTailIndex] = 0x00;

		} else if(!strcmp(pTemp,"set")) { //change data
			//set 1
			//line1 new data
			int nIndex = atoi(strtok(NULL," "));
			free(strLine[nIndex]);
			pTemp = strtok(NULL,"");
			char *pstr = (char *)malloc(strlen(pTemp)+1);
			strcpy(pstr,pTemp);
			strLine[nIndex] = pstr;
		
		} else if (!strcmp(pTemp,"dump")) { // show list
			puts("----------------------------");
			
			for (int i=0; strLine[i] != 0x00; i++) { //0x00 = 0(null)
			puts(strLine[i]);
			puts("----------------------------");
			}

		}

	}

	return 0;
}
